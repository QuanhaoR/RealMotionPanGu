        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        Args:
            hidden_states (`torch.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
                Input hidden states.
            key_padding_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask for padding tokens in key.
            **kwargs:
                Additional arguments for attention implementation.
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        Args:
            ...
            **flash_attn_kwargs:
                Additional flash attention arguments.
                
        Returns:
            tuple containing:
                - hidden_states (`torch.Tensor`): Output hidden states
                - self_attn_weights (`torch.Tensor`, *optional*): Attention weights if output_attentions=True
        """
...
class PanguEmbeddedModel(PanguEmbeddedPreTrainedModel):
    def __init__(self, config: PanguEmbeddedConfig):
        ...
        #--------------------------------------------------------------         begin              --------------------------------------------------------------
        self.config = config
        self.assign_layer_idx = config.assign_layer_idx
        #---------------------------------------------------------------         end               --------------------------------------------------------------
        ...
        self.layers = nn.ModuleList(
            [PanguEmbeddedDecoderLayer(config, self.assign_layer_idx)]# for layer_idx in range(config.num_hidden_layers)]
        )
...
#------------------------------------------------------------------------               begin           -------------------------------------------------------------------   
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        """
        Forward pass of PanguEmbeddedModel.
        
        Args:
            inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`):
                The input embeddings.
            key_padding_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
                Mask to indicate which elements in the input sequence are padding tokens. 
                - `True` tokens are masked (ignored)
                - `False` tokens are attended to
            output_attentions (`bool`, *optional*):
                Whether or not to return the attention tensors of all attention layers.
            **flash_attn_kwargs:
                Additional keyword arguments for flash attention.
        ...
        if inputs_embeds is None:
            raise ValueError("must supply inputs_embeds params")
